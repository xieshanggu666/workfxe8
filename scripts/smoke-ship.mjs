...
console.log('— 种子发货单：已发货购物卡 + 待填地址帆布袋 —')
...
assert(s.shipmentStats.total === 2 && s.shipmentStats.shipped === 1 && s.shipmentStats.pendingAddress === 1,
...
// 库存按既有规则核销：g3 remain 已扣 1
assert(s.goods.find((g) => g.id === 'g3').remain === 49, '帆布袋库存已核销 1（50→49）')
